-- Function to handle user onboarding via Enrollment ID
-- This function runs with SECURITY DEFINER to bypass RLS restrictions
-- allowing a new user to:
-- 1. Find a store by enrollment_id (which they normally can't see)
-- 2. Create/Update their public.users record linked to that store (which they might not have permission to do)
...
BEGIN
  ...
  SET 
    ...
    email = EXCLUDED.email; -- Keep email in sync
